...
describe("Contact", () => {
    ...
    it("Should try to send a message with an invalid email",()=>{
        ...
        cy.get("input").get("#recipient-email").wait(500).type("AbbaIsAGreatGroup");
        cy.get("input").get("#recipient-name").wait(500).type("Abba");
    ...
    it("Should successfully send a message",()=>{
        cy.get(".nav-link").contains("Contact").click();
        cy.get("input").get("#recipient-email").wait(500).type("AbbaIsAGreatGroup@email.com");
        cy.get("input").get("#recipient-name").wait(500).type("Abba");
